   ...
   return (
      <Card className="flex h-full flex-col border-2 transition-all hover:border-primary hover:shadow-md sm:max-w-sm md:max-w-md lg:max-w-lg">
         ...
         <CardFooter className="flex flex-col gap-2 bg-muted/30 p-3 sm:flex-row sm:gap-3 sm:p-4">
            ...
            {liveDemoUrl === '#' ? (
               <Button variant="default" size="sm" className="w-full text-xs sm:text-sm" disabled>
                  <FaLink className="mr-1 h-3 w-3 sm:mr-2 sm:h-4 sm:w-4" />
                  <span className="flex-1">Coming Soon</span>
               </Button>
            ) : liveDemoUrl === 'local' ? (
               <Button variant="default" size="sm" className="w-full text-xs sm:text-sm" onClick={handleLocalDemo}>
                  <FaLink className="mr-1 h-3 w-3 sm:mr-2 sm:h-4 sm:w-4" />
                  <span className="flex-1">Run Locally</span>
               </Button>
            ) : (
               <Button variant="default" size="sm" className="w-full text-xs sm:text-sm" asChild>
